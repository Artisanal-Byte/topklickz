...
const $$WhoWeAre = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<section> <div class="mx-auto max-w-5xl px-4 lg:px-8 py-10 lg:py-20"> <!-- Top Section --> <div class="grid grid-cols-1 gap-5 lg:grid-cols-[0.8fr_1.5fr] lg:gap-24"> <!-- Left --> <div> <h2 class="m-0
						font-clash
						text-[22px]
						font-semibold
						uppercase
...
WHO WE ARE
						...
						from-[#ff9b42]
						via-[#ef2362]
						to-[#6944db]"></div> </div> <!-- Right --> <div class="max-w-[590px] font-geist text-[16px] font-normal leading-[24px] tracking-normal text-[#1d1d1d]"> ${descriptions.map((paragraph, index) => renderTemplate`<p${addAttribute(index === 0 ? "m-0" : "mb-0 lg:mt-5 mt-3", "class")}> ${paragraph} </p>`)} </div> </div> </div> <div class="mx-auto max-w-7xl px-6 lg:px-8"> <!-- Stats --> <div class="border-t border-[#d9d9d9]"> <div class="grid
					grid-cols-1
					sm:grid-cols-2
					lg:grid-cols-5"> ${stats.map((stat, index) => renderTemplate`<div${addAttribute([
    "min-h-[145px] px-5 py-9 sm:px-6 lg:px-5",
    index !== stats.length - 1 && "lg:border-r lg:border-[#d9d9d9]",
    index !== stats.length - 1 && "max-lg:border-b max-lg:border-[#d9d9d9]"
  ], "class:list")}> <h3 class="font-clash-display text-4xl font-[550] leading-[120%] tracking-normal gradient-text lg:text-[45px]"> ${stat.value} </h3> <p class="
									mb-0
									mt-3
									font-satoshi
									text-[15px]
									...
									tracking-[0.84px]
									text-[#858585]
								"> ${stat.label} </p> </div>`)} </div> </div> </div> </section>`;
...
WHAT WE BELIEVE
    ...
    index !== beliefs.length - 1 && "border-b border-[#d2d2d2] md:border-r md:border-b-0",
...
OUR PROCESS
    ...
    index !== processSteps.length - 1 && "border-b border-[#D2D0CB] md:border-b-0",
...
LASTING IMPACT.
    ...
    index % 3 !== 2 && "lg:border-r lg:border-[#D6D3CE]",
    /* desktop first row bottom border */
    index < 3 && "lg:border-b lg:border-[#D6D3CE]",
    /* tablet borders */
    index % 2 === 0 && "md:border-r md:border-[#D6D3CE] lg:border-r",
    index < approachItems.length - 2 && "md:border-b md:border-[#D6D3CE] lg:border-b",
    /* mobile */
    index !== approachItems.length - 1 && "border-b border-[#D6D3CE] md:border-b"
  ], "class:list")}> <h3 class="
								m-0
								max-w-[190px]
								font-clash-display
...
PERSPECTIVES.
    ...
    testimonial.border && "lg:border-l lg:border-[#D5D2CD] lg:pl-7 lg:pl-10"
